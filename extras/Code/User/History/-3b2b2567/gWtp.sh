...
if [[ $user_agreed == "y" || $user_agreed == "Y" ]]; then
    ...
    case "$distro" in
        D|d) read -p "${ACTION} - Installing zsh in your distro\n"
            package_man="apt install"
            ;;
        A|a) read -p "${ACTION} - Installing zsh in your distro\n"
            package_man="pacman -S"
            ;;
        F|f) read -p "${ACTION} - Installing zsh in your distro\n"
            package_man="dnf install"
            ;;
        O|o) read -p "${ACTION} - Installing zsh in your distro\n"
            package_man="zypper install"
            ;;
        V|v) read -p "${ACTION} - Installing zsh in your distro\n"
            package_man="xbps-install"
            ;;
        OB|Ob|read -pprintf "${ACTION} - Installing zsh in your distro\n"
            package_man="pkg_add"
            ;;
        FB|Fb|read -pprintf "${ACTION} - Installing zsh in your distro\n"
            package_man="pkg install"
            ;;
        R|r) read -p "${ACTION} - Installing zsh in your distro\n"
