...
printf "${NOTE} - Now proceeding to the next step '\e[33m[ Configuring $HOME/.zshrc file ]\e[0m'\n"
sleep 2
  ...
  if [ -f ~/.p10k.zsh ]; then
  printf " - Backing up the .p10k.zsh file to .p10k.zsh.back"
    mv ~/.p10k.zsh ~/.p10k.zsh.back
  fi

  sleep 1

    printf " - Copying .zshrc and .p10k.zsh"
    cp -r $HOME/HyprV1/extras/.zshrc ~/
    cp -r $HOME/HyprV1/extras/.p10k.zsh ~/
    printf " - copying finished, making zsh default"