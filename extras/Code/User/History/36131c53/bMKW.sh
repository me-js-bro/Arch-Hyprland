...
install_pack() {
    ...
    if [ $? -eq 0 ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            $ISAUR -Ss $package_name-bin &> /dev/null
            if [ $? -eq 0]; then
                printf "The package your are looking for is available in the AUR named ${green} $package_name-bin${end}. Installing it...\n"
                $ISAUR -S --noconfirm $package_name-bin
            fi
        fi
...
# Take input of the package... ( can be multiple. )
printf "What package you want to install?\n"
read -p "Package Name: " pkg

    for SOFTWR in $pkg; do
